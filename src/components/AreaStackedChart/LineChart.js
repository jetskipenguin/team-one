...
class LineChart extends Component {
  constructor() {
    ...
    render() {
        ...
        const columns=['trade','manufacturing','hospitality','services', 'construction','health','government','finance','semployed','other','utilities','information','agriculture','extraction']
        ...
        const xScale = this.xScale
                ...
                .domain(d3.extent(data, d => d.date))//d3.extend return [min,max]
                // .domain(data.map(d => d.date))
                .range([margins.left, svgDimensions.width - margins.right])

        const yScale = this.yScale
        .domain([0, d3.max(stackedSeries , d => d3.max(d, d => d[1]))]).nice()
